...
def display_sheet(sheet_name, workbook):
    ...
    try:
        ...
        # Xóa các widget cũ trong sheet_inner_frame
        for widget in sheet_inner_frame.winfo_children():
            widget.destroy()
        ...
        end_row = min(current_page * rows_per_page, max_row)

        # Hiển thị tiêu đề cột
        for j in range(1, max_col + 1):
            column_letter = get_column_letter(j)
            header = tk.Label(sheet_inner_frame, text=column_letter, bg="#d3d3d3", font=("Arial", 10, "bold"))
            header.grid(row=0, column=j, padx=2, pady=2)

        # Hiển thị dữ liệu trong phạm vi trang hiện tại
        for i in range(start_row, end_row + 1):
            row_header = tk.Label(sheet_inner_frame, text=str(i), bg="#d3d3d3", font=("Arial", 10, "bold"))
            row_header.grid(row=i - start_row + 1, column=0, padx=2, pady=2)

            for j in range(1, max_col + 1):
                value = sheet.cell(row=i, column=j).value
                cell = tk.Entry(sheet_inner_frame, width=15, font=("Arial", 10))
                cell.grid(row=i - start_row + 1, column=j, padx=2, pady=2)
                cell.insert(0, value if value is not None else "")
                cell.config(state="readonly")

        # Cập nhật lại scroll region của sheet_canvas khi dữ liệu thay đổi
        sheet_canvas.update_idletasks()
        sheet_canvas.configure(scrollregion=sheet_canvas.bbox("all"))

        # Hiển thị số trang
        status_label.config(text=f"Trang {current_page}/{(max_row - 1) // rows_per_page + 1}", fg="blue")
...
def next_page():
    ...
    if filepath:
        try:
            ...
            max_row = sheet.max_row
            if current_page * rows_per_page < max_row:
                current_page += 1
                display_sheet(selected_sheet, wb)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load sheet: {e}")
...
sheet_frame = tk.Frame(root, bg="#f4f4f4", height=600)
sheet_frame.pack(pady=12, fill=tk.X, expand=False)
...
sheet_scroll.pack(side=tk.RIGHT, fill=tk.Y)  # Thanh cuộn dọc chỉ fill chiều dọc
...
pagination_frame = tk.Frame(root, bg="#f4f4f4")
pagination_frame.pack(pady=10)
prev_button = tk.Button(pagination_frame, text="Trang trước", command=previous_page, bg="#0078D7", fg="white", font=("Arial", 10))
prev_button.grid(row=0, column=0, padx=10)
next_button = tk.Button(pagination_frame, text="Trang sau", command=next_page, bg="#0078D7", fg="white", font=("Arial", 10))
